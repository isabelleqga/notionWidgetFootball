...
- **`widget.html`** — self-contained frontend: inline `<style>` (theme-aware via CSS custom properties, redefined under `@media (prefers-color-scheme: dark)`) and inline `<script>` that:
  ...
  - The club/country selector row (`#selector-section`) is hideable via a toggle button (`#toggle-selector-btn`, in the "Matches" section-title row) — `setSelectorVisible()` defaults it hidden on load when a saved selection already exists, so returning users see data immediately instead of the pickers; the toggle always stays reachable to bring it back.
  - There's no standalone page header — the selected club's name is shown as part of the "Matches" section title (`Matches • {club name}`), set in `loadClubData()`.
  - `loadClubData()` is the single entry point that fetches `/api/club-data` and re-renders every section. There's no auto-refresh — data is fetched once when a club is selected/restored and only refetched on an explicit user action (picking a different club) or a page reload.
  - Renders: last-2/next-2 matches (opponent + H/A square only, no "our" team name shown); one consolidated "League" section (titled with the league name) containing three `.subsection`s with subtitles — **Table** (a 5-row window centered on the selected club, clamped at the table's edges — see `displayStandings()`; `table-layout: fixed` + ellipsis truncation keep long club names from overflowing), **Position progress** (a hand-rolled inline-SVG chart, dot color = table zone: blue/title, green/top-4, yellow/mid-table, red/relegation — computed client-side from `teamCount`/`relegationZone`; `renderPositionChart()` draws at the container's actual `clientWidth` rather than a fixed viewBox so dots/text stay legible at any width; every matchday gets a dot, but the numeric label is only drawn on the first point and whenever the position differs from the previous matchday, so an unchanged streak doesn't repeat the same number), and **Form** (win/draw/loss/upcoming squares, which shrink via `flex-shrink`/`aspect-ratio` on `.squares-rows .sq` rather than overflowing a narrow card); a separate Champions League section follows the same title + "Form" subtitle pattern when the club is in it this season.
  - A `@media (max-width: 380px)` breakpoint (tighter padding, stacked selector row, smaller type) keeps the widget usable when a Notion page is split into several side-by-side columns (a column can be as narrow as ~240px).
  - **Layout mode picker**: a ⚙ button (`#layout-settings-btn`, grouped with the selector toggle in the Matches title row) opens a menu to switch `#content-sections` between three modes, persisted in `localStorage` (`football-widget-layout-mode`, default `scroll`):
    - `scroll` — today's stacked layout, unchanged.
    - `tabs` — a tab bar shows one of Matches/League/Champions League at a time; the CL tab only appears when applicable and falls back to Matches if the active tab stops being applicable.
    - `side-by-side` — `#content-sections` becomes a CSS grid (`repeat(auto-fit, minmax(240px, 1fr))`) that collapses to stacked on narrow widths, and `#container` drops its 600px cap (`.mode-wide`) so the grid can actually use the embed's real width. In this mode only, the League section unwraps (`display: contents`) so its two `.league-column` groups — Table, and Position progress + Form — become independent grid cards instead of one big "League" card; Scroll/Tabs are unaffected.
...
- README exists in two languages, kept in sync by hand (GitHub markdown has no real i18n/tabs mechanism): `README.md` (English) and `README.pt-BR.md` (Portuguese/Brazil), cross-linked via a "Languages:" line at the top of each. Any README content change should be mirrored in both.
